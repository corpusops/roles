...
version={{corpusops_services_db_mysql_version}}
...
if has_command netstat;then
    ...
    socket_path=$(grep mysql /proc/$pid/net/unix|awk '{print $NF}')
else
	socket_port=5432
fi
if [[ -z $socket_path ]];then
    ...
    socket_path="/var/run/mysql/.s.PGSQL.$socket_port"
fi
if [[ ! -e "$binary" ]];then
...
export PGHOST="/var/run/mysql"
export PGPORT="$socket_port"
shift
export PATH=/usr/lib/mysql/$version/bin:$PATH
exec $binary $@
