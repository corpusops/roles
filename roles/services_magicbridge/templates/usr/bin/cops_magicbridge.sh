...
#   - cp $this_script  /some/where/cops_magicbridge_lxcfoo
#   * OPT:
#      - $ED /etc/default/cops_magicbridge_lxcfoo if neccesary
#      - $ED /etc/dnsmasq.lxcfoo/*
#   - /some/where/cops_magicbridge_lxcfoo start | stop

# get_bridge foo
# mgcbr0
# get_bridge /usr/bin/cops_magicbridge_tata
# tata
get_bridge() {
    sc=${1}
...
NET="${MAGICBRIDGE_NET:-"10.4.0.0"}"
DEFAULTS="${distrosysconfdir}/cops_magicbridge_${BRIDGE}"
...
set_vars() {
    ...
    LOCKFILE="${LOCKDIR}/cops_magicbridge-${BRIDGE}"
    # relink interfaces with their bridges if the script exists
    RELINK_SCRIPT=""
    for i in /etc/network/if-up.d/cops_reset-net-bridges /etc/cops_reset-net-bridges;do
        if [ -e "${i}" ];then
            RELINK_SCRIPT="${i}"
            break
        fi
...
teardown_forward() {
    ...
    for proto in tcp udp icmp;do
        iptables_remove OUTPUT -o ${BRIDGE} -p ${proto} -j ACCEPT
    done
...
setup_forward() {
    ...
    if [ "x${IPTABLES_DISABLED}" = "x" ];then
        ...
        for proto in tcp udp icmp;do
            iptables_add OUTPUT -o ${BRIDGE} -p ${proto} -j ACCEPT
        done
