...
Execute burp backup, retry a bit on common errors
...
LOG = "/var/log/burp-client-{{data.name}}"
LOCK = "/var/run/burp-client-{{data.name}}.py.lock"
DEFAULT_TIMEOUT = 2 * 23 * 60 * 60
DEFAULT_DELAY = 5
CMDS = ['burp -a t -c {0} 2>&1']
OPTIONS = {
    'delay': DEFAULT_DELAY,
}
...
def get_worker_pids(*args, **kwargs):
    ...
    ops = popen(
        ...
        '|grep -v \'sh -c\''
        '|awk \'{print $2}\'')[0]
    out = _decode(ops[0]) + _decode(ops[1]) + "\n"
    ...
    chunk_re = re.compile('[:-]')
    if pids:
        details = _decode(popen('ps -eo pid,etime,comm,args')[0][0]).splitlines()[1:]
        for line in details:
            infos = line.split()
            pid = infos[0].strip()
            if pid not in pids:
...
def get_deployer_pids(*args, **kwargs):
    ...
    pps = popen(
        ...
        {% raw %}'|awk \'{{print $2}}\''.format(prog))[0]{%endraw%}
        ...
        while do_loop:
            for cmd in CMDS:
                try:
                    ...
                    while True:
                        try:
                            content = _decode(streamout_queue.get_nowait())
                            if buf:
                                content = buf + content
                                buf = ''
                            try:
                    ...
                    while True:
                        try:
                            content = _decode(streamerr_queue.get_nowait())
                            if buf:
                                content = buf + content
                                buf = ''
                            try:
...
def main():
    ...
    parser.add_option("--burp-config",
                      dest="burp_config", default="/etc/burp/burp.conf")
